...
export default function Toast({ toast, onRemove }) {
  ...
  return (
    <div 
      className={`
        flex items-center p-3 rounded-lg shadow-lg mb-2 text-neutral-900
        ${toast.type === TOAST_TYPES.ERROR ? 'text-red-900' : ''} 
      ...
      <div className="flex-1">
        ...
        {toast.action && (
          <button 
            ...
            className="mt-1 px-3 py-1 bg-neutral-900 bg-opacity-25 hover:bg-opacity-40 rounded text-sm font-medium transition-colors duration-200"
          >
            {toast.action.text}
          </button>
        )}
